...
```bash
npm install
cp .env.example .env   # fill in DISCORD_TOKEN, CLIENT_ID, GUILD_ID, SQLITECLOUD_URI
...
Single-guild Discord bot. Game state lives in-memory (`activeSessions` Map in `src/commands/taixiu.js`). A bot restart clears all active game sessions.
...
- **Deploy-commands.js uses guild-scoped commands** (`Routes.applicationGuildCommands`), not global. Commands only appear in the configured GUILD_ID.
